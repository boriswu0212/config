...
seg_model() {
  [ -n "$model" ] || return 0
  local label="${model#Claude }"
  label="${label//(1M context)/[1m]}"
  fg "$VL_FG_TEXT"
  push "$VL_BG_MODEL" "${BOLD}${_FG} ◆ ${label} ${NORM}"
}

seg_ctx() {
  [ -n "$ctx_pct" ] || return 0
  local ci fgc fgd ti to cache_rate
  ...
  if [ "${tok_in:-0}" -gt 0 ] 2>/dev/null; then
    cache_rate=$(( (${tok_cr:-0} * 100) / tok_in ))
  else
    cache_rate=0
  fi
  push "$VL_BG_CTX" "${fgc} ctx ${ci}% ${fgd}↑${ti} ↓${to} cache:${cache_rate}% "
}
